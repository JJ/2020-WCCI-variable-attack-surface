...
In our case, and like \cite{john_evolutionary_2014}, we have chosen to work on defending web services by optimizing the configuration of a web server unlike he did, we
have chosen the {\sf nginx} static web server and proxy. In our previous papers \cite{erseco:evostar}, we realized that one of the main challenges
was to define a fitness function that would be able to reflect changes
in the vulnerabilities exposed by the site, even subtle ones. This paper will be mainly
devoted to examine a new fitness function that takes into
account the severity of the vulnerability raised, together with several piecewise improvements to our
previous system. We will briefly present the methodology we have
followed next.
