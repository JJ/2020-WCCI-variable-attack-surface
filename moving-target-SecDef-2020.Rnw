...
This procedure can be generalized to any kind of service, as long as configurations can be evaluated automatically (which they can, most of the times) and optimal configurations can be found with relative ease; keeping the evolutionary algorithm running, and extracting all solutions that are secure, will keep the moving target defense moving, so to say.
