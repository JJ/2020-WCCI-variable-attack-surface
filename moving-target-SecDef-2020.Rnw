...
in the vulnerabilities exposed by the site, even subtle ones. Improving on results published in \ref{mtd:gecco:anon}, that introduced a new fitness function that takes into account severity of vulnerabilities, in this paper we will show how the evolutionary algorithm can provide a steady supply of new configurations by just letting it run for a number of generations. Since the optimum is found in just a few generations, running past the point the whole population has reached zero medium vulnerabilities will provide system administrators or chief security officers with a good amount of secure configurations for their servers.
