...
\copyrightyear{2020}
\acmYear{2020}
\setcopyright{acmlicensed}\acmConference[GECCO '20 Companion]{Genetic
and Evolutionary Computation Conference Companion}{July 8--12,
2020}{Cancún, Mexico}
\acmBooktitle{Genetic and Evolutionary Computation Conference Companion
(GECCO '20 Companion), July 8--12, 2020, Cancún, Mexico}
\acmPrice{15.00}
\acmDOI{10.1145/3377929.3398083}
...
\title[Delivering diverse web configurations in a MTD using EAs]{Delivering diverse web server configuration in a moving target defense using evolutionary algorithms}
...
\keywords{Security, cyber-attacks, performance evaluation, moving
...
Defending a system against cyber-attacks includes many different
techniques; the vulnerability of the system needs to be minimized, a procedure usually called {\em hardening}, which is a problem that can effectively be formulated as an optimization one. A hardened objective might still be attacked, as
long as the person attacking is able to profile it and try different
things on it, simultaneously or later on. Avoiding this kind of profiling is the main
objective of the so called Moving Target Defense, or MTD
\cite{moving-target,jajodia2011moving,Cai2016,sengupta2020survey}.

Moving target defense does not specify either the kind of attack, the
...
service defended in that way after a random amount of time (or after an attack has been detected) so that it
creates different traffic and response patterns that make impossible
to identify a node that does a specific service in the network. In our work we have chosen to work on defending web services (or web
sites) by optimizing the configuration of a web server, following the
work of \cite{john_evolutionary_2014}, which was one of the first to apply evolutionary algorithms to evolve web server configurations that can be used in a moving target defense; unlike they did, we
have chosen the {\sf nginx} static web server and proxy, while he
focused on the well known Apache web server. Our choice was motivated
by the fact that nowadays {\sf nginx} is more popular, and still
growing in popularity nowadays; anyway, configuration of web servers is similar in a number of ways and whatever results are obtained can be applied to that or other web servers like Caddy.

In our previous papers \cite{erseco:evostar,erseco:cec,mtd:gecco}, we realized that one of the main challenges
was to define a fitness function that would be able to reflect changes
in the vulnerabilities exposed by the site, even subtle ones. Improving on results published in \cite{mtd:gecco}, that introduced a new fitness function that takes into account severity of vulnerabilities, in this paper we will show how the evolutionary algorithm can provide a steady supply of new configurations by just letting it run for a number of generations. Since the optimum is found in just a few generations, running past the point the whole population has reached zero medium vulnerabilities will provide system administrators or chief security officers with a good amount of secure configurations for their servers. Our objective is to run a single evolutionary algorithm, and after a few generations (and minutes) use generated configurations with zero medium vulnerabilities to change configuration in a moving target defense.
...
We chose to focus on the moving target defense because it is a very popular technique, with dozens of new
papers published every year, including a yearly conference, the
Workshop on Moving Target Defense; recent surveys include
\cite{Cai2016,Larsen201428,lei2018moving,ward2018survey,cho2019toward,sengupta2020survey,TORQUATO2020101742}. A
recent survey published by Zheng and Namin \cite{Zheng2019207}
explains the kind of attacks that MTD can defend against and introduces the concept of entropy as the key measurement of the
unpredictability of the defense policy. The best defense should be done at a network level, but this can be complemented by service-level MTD
such as the one we propose in this paper.


We will be using evolutionary algorithms in this paper, so we will focus on bioinspired approaches to this methodology, which were used from the beginning: a non-affiliated
...
as Nessus. The fact that cyberdefense must evolve with threats makes researchers revisit this kind of model; recently, Ahmed et al. \cite{ahmed2020bio} proposed a similar one for creating diverse virtual machines in a cloud computing environment.x

Our work has been initially inspired by John et
al. \cite{john_evolutionary_2014}, which was also presented in the SecDef 2014 workshop. John and coauthors use an evolutionary algorithm for a a host-level
...
server using the so called CVSS score \cite{cvss}, which is computed from 
...
solutions to these two problems, which was briefly introduced in \cite{mtd:gecco}. In our work we will try to apply the
...
As in our previous papers \cite{erseco:evostar,erseco:cec,mtd:gecco}, we have chosen
...
\cite{erseco:evostar,erseco:cec}, from where this section is largely taken
and expanded.
...
\item \texttt{keepalive\_timeout}: Period during which a server-side
  connection from a client will remain open  before timing out.
...
\label{subs:setup}
 ...
 This paper uses the same method as described in \cite{mtd:gecco}, where it was omitted for brevity. 
The most important part of the evolutionary algorithm is designing
correctly the fitness function that is going to be tested and used. A configuration is meaningless without
content behind, and we need to choose what is going to be the
content. As in our previous papers
\cite{erseco:evostar,erseco:cec}, we have used a basic
...
previous papers \cite{erseco:evostar,erseco:cec}, in this paper
...
Performing the experiments consists then in firing up, using docker-compose, the three docker containers used, starting an evolutionary algorithm from one of the containers, and collecting information in a text log file. The logs of all experiments are available, under a free license, from the GitHub repository mentioned above.
...
implementation of the evolutionary moving target defense method. In \cite{mtd:gecco} we published results for a population of 16; however, we expected to obtain better results with a population of 32, since more population implies more diversity.
...
\cite{erseco:evostar,erseco:cec} to this paper is that, in this case,
there is a very low likelihood of finding a chromosome in the last generation with medium
...
before, in most cases all chromosomes share this low vulnerability score.

We can observe that in Figure \ref{fig:vulnerabilities}.
%
\begin{figure}[h!tb]
...
@
\caption{Boxplot of the number and class of vulnerabilities across 15 executions of the evolutionary algorithm for population = 16 (top) and 32 (bottom); it shows the average number of medium and low-risk vulnerabilities per generation.}
  \label{fig:vulnerabilities}
\end{figure}

This figure, that represents the average across 15 runs, shows how medium-risk vulnerabilities are essentially 0 after the third generation, with more steep descent for the bigger-size population, 32; this proves that using a fitness function based on the type, and not only the number, of vulnerabilities, makes the population reach a low vulnerability score really fast. But the real improvement lays in the use of the new sorting function 
to select chromosomes. In \cite{mtd:gecco} we have shown that a population of 16 (with 32 generations) is already enough to generate a high variability; here we represent, in Figure \ref{fig:md}, average distances for population equal to 32.
%
\begin{figure}[h!tb]
...
\caption{Average mutual distance for individuals in the last
  generation for experiments with population 32. GECCO labels the ones that use the new sort method, while
  "Sorted" uses the same algorithm, but the whole chromosome, not only
the fitness, is sorted.}
  \label{fig:md}
\end{figure}
%
...
affirm that there's a good average distance between them, which makes them much more useful in a MTD context; this
...
\caption{Population-level mutual distance entropy for individuals in the last
  generation for experiments with population 32. Labels as above, with GECCO tagging the ones we have
...
moving target defense. It has been, along with \cite{mtd:gecco}, the result of progressive
...
The result of changing the fitness function has been to eliminate
configuration values that created medium-severity vulnerabilities is that the evolutionary can be used to be run continuously, and provide a steady supply of serviceable configurations in a moving target defense; the key factors to achieving this was, besides using a fitness function that distinguishes low and medium level vulnerabilities, to use a selection procedure that preserves the randomness present in the population in a flat fitness landscape, by not using always the first individuals sorted in some non-changing order, but giving all individuals with the same fitness a chance to reproduce.

This procedure can be generalized to any kind of service, as long as configurations can be evaluated automatically (which they can, most of the times) and optimal configurations can be found with relative ease; keeping the evolutionary algorithm running, and extracting all solutions that are secure, will keep the moving target defense moving, so to say. In both cases, this will be more efficient that a systematic change (or a random one) by an expert, since new configurations generated are guaranteed to be optimal (at least in most cases in the population).
...
put up an actual web site. A static analyzer of the configuration will also void the need for a expensive setup and run by yielding a vulnerability score immediately. Some applications like {\tt gixy} are able to do that for a limited amount of configurations. 
