...
al. \cite{john_evolutionary_2014}, which was also presented in the SecDef 2014 workshop. John and coauthors use an evolutionary algoritm for a a host-level
...
This figure, that represents the average across 15 runs, whows how medium-risk vulnerabilities are essentially 0 after the third generation, with more steep descent for the bigger-size population, 32; this proves that using a fitness function based on the type, and not only the number, of vulnerabilities, makes the population reach a low vulnerability score really fast. But the real improvement lays in the use of the new sorting function 
...
put up an actual web site. A static analyzer of the configuration will also void the need for a expensive setun and run by yielding a vulnerability score inmediately. Some applications like {\tt gixy} are able to do that for a limited amount of configurations. 
