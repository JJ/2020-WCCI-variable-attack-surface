...
solutions to these two problems, which was briefly introduced in \cite{mtd:gecco:anon}. In our work we will try to apply the
...
As in our previous papers \cite{erseco:evostar:anon,erseco:cec,mtd:gecco:anon}, we have chosen
...
\label{subs:setup}
 ...
 This paper uses the same method as described in \cite{mtd:gecco:anon}, where it was omitted for brevity. 
...
implementation of the evolutionary moving target defense method. In \cite{mtd:gecco:anon} we published results for a population of 16; however, we expected to obtain better results with a population of 32, since more population implies more diversity.
...
to select chromosomes. In \cite{mtd:gecco:anon} we have shown that a population of 16 (with 32 generations) is already enough to generate a high variability; here we represent, in Figure \ref{fig:md}, average distances for population equal to 32.
%
\begin{figure}[h!tb]
...
moving target defense. It has been, along with \cite{mtd:gecco:anon}, the result of progressive
