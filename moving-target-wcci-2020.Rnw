...
Our previous paper \cite{erseco:evostar}ww was a proof of concept and
...
\cite{erseco:evostar}; nine of then are {\sf nginx} directives
(shown in Table \ref{table:nginx_directives}) and 6 are set from the same
configuration file, but affect HTTP headers (shown in Table
\ref{table:http_headers}), which in turn affect how the browser
receving the information behaves. This subset is extracted from the DISA STIG recommendations
for hardening webservers based in the CVSS score \cite{disa20:apache}. Most of these values
are defined in the original document as Apache HTTP server configuration values but have a
...
\end{table}
%
The web servers also send a number of headers, whose value can be
configured from the {\sf nginx} configuration file. These are
...
The problem of optimizing security in configuration is two fold: some
...
additionale, a intentionally vulnerable application, OWASP's Juice
...
The evaluation is then performed in the following sequence:\begin{itemize}
...
The more time-consuming part of this process is the scanning, which
...
have to use only active scanning, the same as in our previous paper.


\begin{figure*}[h!tb]
  
  \includegraphics[width=0.9\linewidth]{fitness-evolution-32-32.png}
  \caption{3D mesh representation of evolution of fitness, with z axis
    ...
    representing generations; evolution proceeds from left to right.}
  \label{fig:evolution}
  \end{figure*}
%
As in our previous paper, \cite{erseco:evostar}, we have designed the
rest of the algorithm in Python, to be able to acommodate the ZAPpy
...
evolution, which was probably due to a naïve implementation of the
...
previous paper \cite{erseco:evostar} in this paper we have tried to
...
used a {\em incremental} mutation, that adds or substracts one from
...
one or subsctracted one); if the value of {\tt worker\_connections} is
...
previous naïve one, so we explored its result by making some test runs
...
results, and that the evolutionary agorithm is able to overcome, at
...
\begin{table*}
\centering
\caption{Experiment results for every run made for population 16 and 32.}
...
% Future lins of work.
In the immediate future, now that the evolutionary algorithm is
