...
{\em Zep Attack Proxy} (ZAP), an open source tool that gives as a score for an installation the
...
program to improve the cyberdefense skills in America. MTD is targeted % America -> USA? - Mario
...
\cite{jajodia2011moving}, and thus making attacks against it either too
