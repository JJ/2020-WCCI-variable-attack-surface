...
followed by the experimental results, which we will discuss in Section \ref{sec:res}
...
From the more than 700 directives available in {\sf nginx} we chosen nine
...
paper 6 HTTP headers (shown in Table \ref{table:http_headers}), which affects
how the browser process the received pages.
This subset of 15 is extracted from the more extense DISA STIG recommendations
...
but we look for a {\sf nginx} equivalent directive.
...
When off (default value) accessing a file
that is a symbolic link raises a denied access error.
\item \texttt{autoindex}: Allows showing a page listing the content of the
current directory. Set off by default.
\item \texttt{send\_timeout}: Defines the maximum allowed time to transmit a
response to the client. 60 seconds by default.
...
leaving a default number of 4 buffers.
\item
\texttt{client\_max\_body\_size}: The maximum size allowed per a client request
body. If the client exceed this value the server will return an error.

\item \texttt{server\_tokens}: Return some server info in the {\tt Server}
response header. By default show the {\sf nginx} version.
\item
\texttt{gzip}: Enables the compression of HTTP responses. It's not a security
...
({\tt none}), or including google domains {\tt hosts:*.google.com}, in
