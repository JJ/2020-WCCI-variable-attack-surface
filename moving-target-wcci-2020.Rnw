...
content. In our previous paper \cite{evostar:erseco}, a basic application was created, and
...
As in our previous paper, \cite{erseco:evostar}, we have designed an
evolutionary algorithm in Python that is able to minimize the ZAP
score of the chosen sites. 
