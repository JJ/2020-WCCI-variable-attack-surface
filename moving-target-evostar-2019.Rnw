...
Our moving target defense approach relies on the diversity of each generated configuration to improve the security of the our system. This is based on the entropy of our random generated and mutated values, but this random configuration can be wrong or vulnerable so we test that configuration and by the genetic algorithm evolve that configuration to get a good one.
