...
vulnerabilities \cite{yang2012security}: The `Security Technical Implementation Guides' or STIGs are the configuration standards for DoD systems provided by the `Defense Information Systems Agency' (DISA) since 1998. These guides give some recommendations to hardening the configuration of software systems based on known vulnerabilities and his impact is classified using the CVSS score.
...
yielding a scalar value based on the number of vulnerabilities found in a site.
...
% 3.
Early MTD techniques included the use of rotating virtual webservers
...
(such as nginx and Apache), combined with plug-ins\footnote{It should
  ...
  applications}. MTD can also be applied to software defined networks
\cite{al2011toward}. 
...
\caption{Scatter plot for OWASP ZAP scores for every experiment using the
  ...
  (proportion of the population equal to the highest fitness == 1). }
...
population has the same low vulnerability score (proportion equal to 1).}
\label{fig:zap:static}
\end{figure*}
