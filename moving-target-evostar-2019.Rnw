...
Nginx. In recent years Nginx has surpassed Apache as the most used
...
The last stable version of Nginx (1.17) has more than 700
...
Nginx directives (\ref{table:nginx_directives}) and 6 HTTP headers
...
takes to generate a set of different Nginx configurations with low
...
We will discuss this results next.
