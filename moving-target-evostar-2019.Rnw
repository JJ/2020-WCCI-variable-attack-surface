...
After generating the population we calculate the fitness of that population using Zap that gives a scalar value with the number of known vulnerabilities a configuration has, we sort the population list in reverser order to set the better ones at the end of the list and get the p (pressure) values that we will evolve during 15 generations.

For evolving the configuration we are using a crossover in one point and crossover in two-point functions.
