...
cybersercurity world, the whole system is released under a free
software license.
...
are defined as Apache HTTP server configurationv values but have a
...
Enable or disable the broadcast of the {\sf nginx} version on the error pages and in the `Server' response header. It is recommended not giving too extensive informatión of software versions, but we can cheat the attacker telling wrong server version info.
\item 
\texttt{gzip}:
...
The genetic algorithm works generating a population of $n$ individuals. Each individual is a chromosome of 15 gens and each gen refers to an {\sf nginx} directive or an HTTP security header. 
...
every experiment, are available in GitHub and are in fact included in
...
\end{figure*}
%
Since the MTD is based on the frequent and impredictable changing of
...
of vulnerabilities. First it should be noted that acceptables ZAP
...
the vulnerability. However, in general, the objecties of this paper
have been reached.
...
expand the search space of the evolutionary algorith, and this can be
...
some machine learning model that is able to inmediately issue a score
...
something that can be done inmediately, and will be one of our next steps.
