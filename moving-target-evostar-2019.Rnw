...
While CVSS is an abstract way to score security, we first need to decide on a system for which security is going to be optimized and, second, on a tool that will be able to automatically compute CVSS. For the former, we will use an open-source OWASP project called Juice-Shop \cite{luburic2019utilizing,juice-shop} that consists in a vulnerable e-commerce platform written in Node.js, Express, and Angular. This is one of the most typical web application configurations, being more complex than a simple welcome webpage and more similar to a real environment

For the latter, that is, to quantify the security of a given configuration we can use tools like OWASP ZAP \cite{bennetts2013owasp}. OWASP ZAP is an open-source security analysis tool  for web applications developed by OWASP,  crawls and analyzes a specified site for security vulnerabilities, yielding a scalar value based on CVSS score. 

Our moving target defense approach relies on the diversity of each generated configuration to improve the security of our system. This is based on the entropy of our random generated and mutated values but this random configuration can be wrong or vulnerable so we test that configuration and by the genetic algorithm evolve that configuration to get a good one.
The same process is be applied to the resulting configurations to improve the security through generation, but as we more evolve the configuration we get a much more secure system but we lose diversity. We need to balance security and diversity depending on our system.
...
Bioinspired metaheuristics have been often used in the area of
cybersecurity; for instance, some authors have proposed using
...
speculative execution feature. This type of defense was approached
using evolutionary algorithms for the first time by John et
al. \cite{john_evolutionary_2014}. More recently, Buji et al. in
\cite{buji_genetic_2019} % Add something here - JJ
, but also applied to reconfiguration of
software defined networks by Champagne et al. in
\cite{champagne_genetic_2018}. 
...
For the fitness function, we are using the OWASP ZAP Python API calling a container with the Docker version of OWASP ZAP simulating a real environment using the example.com domain and the generated configuration. 

The OWASP ZAP Python API will give the mentioned scalar value depending on the number of known vulnerabilities of that configuration.
...
For the 16 individuals population size the experiment took an average of 35 minutes, taking 80 minutes in the 32 individuals population size and 180 minutes for the 64 individuals population size. This times are the reason of running each population size in different EC2 instances. The running time of all instances was 266 hours equivalent to 11 days of total processing time having a total cost of \$11.07.

We will run 15 times the experiments for each 16, 32 and 65 population size, trying the different crossover functions and the different mutation methods to know which one is giving better results. As we said previously, we are searching for a balance between diversity and secure configurations to apply in the final server. 

The experiment commands are the next ones:

\begin{verbatim}
python3 genetic.py --individuals POPULATION_SIZE --crossover-one-point --random-mutation
python3 genetic.py --individuals POPULATION_SIZE --crossover-one-point --no-random-mutation
python3 genetic.py --individuals POPULATION_SIZE --crossover-two-points --random-mutation
python3 genetic.py --individuals POPULATION_SIZE --crossover-two-points --no-random-mutation  
\end{verbatim}
