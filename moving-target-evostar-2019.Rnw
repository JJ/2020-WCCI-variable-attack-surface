...
population using Zap, which gives a scalar value with the number of
...
For the fitness function, we are using the ZAP Python API calling a container with the Docker version of OWASP ZAP simulating a real environment using the example.com domain and the generated configuration.

The ZAP Python API will give the mentioned scalar value depending on the number of known vulnerabilities of that configuration.

We ran the experiments in three AWS EC2 t3.medium instances all running Ubuntu 18.04 LTS with Docker installed, each instance has 2 vCPU and 4 GiB of RAM. Each instance run a different set of experiments of 16, 32 and 64 population size. To orquestate the instances we used a simple Ansible playbook.
...
@
\caption{Scatter plot for ZAP scores for every experiment using the
  static web site }
...
@
\caption{Scatter plot for ZAP scores for every experiment with the
  Juice Shop site }
\label{fig:zap:static}
\end{figure*}
