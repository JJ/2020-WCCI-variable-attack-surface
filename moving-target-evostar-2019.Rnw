...
For evolving the configuration we are using a crossover in one point
and crossover in two-point functions.

Also, we are mutating the population with a chance of 0.4 using two different methods, changing random gen with a random correct value or increasing/decreasing random gen.
...
We run 15 times the next experiments for each 16, 32 and 65 population size:
