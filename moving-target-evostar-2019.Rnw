...
The MTD is based on the frequent and impredictable changing of
configurations, so the first thing we need to asses is how long it
...
\end{figure*}
%
This is shown in Figure \ref{fig:zap:static}, which represents the results for the static website.
