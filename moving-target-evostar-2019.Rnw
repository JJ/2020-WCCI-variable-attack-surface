...
After generating the population we calculate the fitness of that population using Zap that gives a scalar value with the number of known vulnerabilities a configuration has.

After getting the fitness values of all population we sort it and get the p (pressure) values that we will evolve during 15 generations.
...
We ran the experiments in three AWS EC2 t3.medium instances all running Ubuntu 18.04 LTS with Docker installed, each instance has 2 vCPU and 4 GiB of RAM.

Each instance run a different set of experiments of 16, 32 and 64 population size. To orquestate the instances we used a simple Ansible playbook.
