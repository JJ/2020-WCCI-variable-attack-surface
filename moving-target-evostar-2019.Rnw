...
Enable or disable the broadcast of the Nginx version on the error pages and in the `Server' response header.

\texttt{gzip}:
Enable or disable the compression of HTTP responses.
...
\texttt{X-Powered-By}:
The `X-Powered-By 'header is used to specify the software that generated the response. It is recommended not giving too extensive information in this header because can reveal details that can facilitate the task of finding and exploiting security flaws.
...
\texttt{server}:
The `Server 'header contains information about the software used by the server.
